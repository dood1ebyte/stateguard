...
def _find(violations: list[Any], field_path: str) -> Any:
    ...
    assert len(matches) == 1, (
        f"Expected exactly one violation for '{field_path}', found {len(matches)}: {matches}"
    )
    return matches[0]
...
class TestEmptyAndTrivial:
    def test_empty_contract_empty_data_is_valid(self, validator: ContractValidator) -> None:
        contract = ContractSpec(fields=[])
        result = validator.validate(contract, {})
        assert result.is_valid is True
        assert result.violations == []

    def test_empty_contract_with_data_strict_false(self, validator: ContractValidator) -> None:
    ...
    def test_empty_contract_with_data_strict_true(self, validator: ContractValidator) -> None:
...
class TestExactMatch:
    ...
    def test_integer_value_for_float_field_is_valid(self, validator: ContractValidator) -> None:
...
class TestMissingRequiredField:
    def test_single_missing_required_field(self, validator: ContractValidator) -> None:
        contract = ContractSpec(fields=[FieldSpec("temperature", FieldType.FLOAT)])
        result = validator.validate(contract, {})
        assert result.is_valid is False
        assert len(result.violations) == 1
    ...
    def test_multiple_missing_required_fields(self, validator: ContractValidator) -> None:
        contract = ContractSpec(
            fields=[
                FieldSpec("temperature", FieldType.FLOAT),
                FieldSpec("humidity", FieldType.INTEGER),
    ...
    def test_optional_field_missing_is_not_a_violation(self, validator: ContractValidator) -> None:
        contract = ContractSpec(
            fields=[
                FieldSpec("temperature", FieldType.FLOAT),
...
class TestUnexpectedField:
    def test_unexpected_field_non_strict_is_warning(self, validator: ContractValidator) -> None:
        contract = ContractSpec(fields=[FieldSpec("temperature", FieldType.FLOAT)])
        result = validator.validate(contract, {"temperature": 30.0, "temp_celsius": 30.0})
    ...
    def test_unexpected_field_strict_is_error(self, validator: ContractValidator) -> None:
        contract = ContractSpec(
    ...
    def test_multiple_unexpected_fields(self, validator: ContractValidator) -> None:
    ...
    def test_weather_drift_scenario(self, validator: ContractValidator) -> None:
        ...
        result = validator.validate(contract, {"temp_celsius": 31.5, "humidity": 80})
        assert result.is_valid is False
        types = _violation_types(result.violations)
...
class TestTypeMismatch:
    ...
    def test_string_where_float_expected(self, validator: ContractValidator) -> None:
        contract = ContractSpec(fields=[FieldSpec("temperature", FieldType.FLOAT)])
    ...
    def test_null_field_type_with_none_value_is_valid(self, validator: ContractValidator) -> None:
...
class TestNullHandling:
    ...
    def test_not_null_constraint_satisfied(self, validator: ContractValidator) -> None:
        contract = ContractSpec(
            fields=[
                FieldSpec(
                    "user_id",
    ...
    def test_none_without_not_null_constraint_is_valid(self, validator: ContractValidator) -> None:
...
class TestValueConstraints:
    ...
    def test_minimum_satisfied_at_boundary(self, validator: ContractValidator) -> None:
        contract = ContractSpec(
            fields=[
                FieldSpec(
                    "age",
    ...
    def test_maximum_satisfied_at_boundary(self, validator: ContractValidator) -> None:
        contract = ContractSpec(
            fields=[
                FieldSpec(
                    "percent",
    ...
    def test_min_length_satisfied_string(self, validator: ContractValidator) -> None:
        contract = ContractSpec(
            fields=[
                FieldSpec(
                    "name",
    ...
    def test_max_length_satisfied_string(self, validator: ContractValidator) -> None:
        contract = ContractSpec(
            fields=[
                FieldSpec(
                    "code",
    ...
    def test_constraint_skipped_on_type_mismatch(self, validator: ContractValidator) -> None:
        """
    ...
    def test_multiple_constraints_all_checked(self, validator: ContractValidator) -> None:
        contract = ContractSpec(
            fields=[
                FieldSpec(
                    "age",
...
class TestNestedSchemas:
    ...
    def test_missing_nested_required_field(self, validator: ContractValidator) -> None:
        contract = self._address_contract()
        result = validator.validate(
            contract,
            {
    ...
    def test_nested_unexpected_field_non_strict(self, validator: ContractValidator) -> None:
    ...
    def test_nested_unexpected_field_strict(self, validator: ContractValidator) -> None:
    ...
    def test_two_level_nesting_missing_deep_field(self, validator: ContractValidator) -> None:
        country_spec = ContractSpec(fields=[FieldSpec("code", FieldType.STRING)])
        address_spec = ContractSpec(
            fields=[
                FieldSpec("city", FieldType.STRING),
    ...
    def test_optional_nested_object_absent_is_valid(self, validator: ContractValidator) -> None:
        inner = ContractSpec(fields=[FieldSpec("city", FieldType.STRING)])
        contract = ContractSpec(
            fields=[
                FieldSpec("name", FieldType.STRING),
                FieldSpec("address", FieldType.OBJECT, required=False, nested_spec=inner),
            ]
        )
        result = validator.validate(contract, {"name": "Alice"})
        assert result.is_valid is True
    ...
    def test_object_field_without_nested_spec(self, validator: ContractValidator) -> None:
...
class TestArrayFields:
    ...
    def test_empty_array_valid_without_min_length(self, validator: ContractValidator) -> None:
        contract = ContractSpec(
            fields=[FieldSpec("tags", FieldType.ARRAY, item_type=FieldType.STRING)]
        )
    ...
    def test_array_without_item_type_accepts_mixed(self, validator: ContractValidator) -> None:
...
class TestNonHalting:
    def test_all_violation_types_collected_simultaneously(
        ...
        contract = ContractSpec(
            fields=[
                ...
                FieldSpec(
                    "nested_obj", FieldType.OBJECT, nested_spec=nested
                ),  # STRUCTURAL_MISMATCH
            ]
        )
    ...
    def test_violation_count_matches_problems(self, validator: ContractValidator) -> None:
        contract = ContractSpec(
            fields=[
...
class TestIsValidSemantics:
    def test_warning_only_is_still_valid(self, validator: ContractValidator) -> None:
        contract = ContractSpec(fields=[FieldSpec("a", FieldType.STRING)])
